...
const ApplicationList = () => {
  ...
  const fetchApplications = async () => {
    try {
      ...
      const res = await fetch("https://facultyms-be-3.onrender.com/leaves", {
        method: "GET",
        credentials: "include",
      });
  ...
  const updateStatus = async (id, status) => {
    try {
      const res = await fetch(`https://facultyms-be-3.onrender.com/leaves/${id}/status`, {
        method: "PUT",
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`https://facultyms-be-3.onrender.com/leaves/${id}`, {
        method: "DELETE",
        credentials: "include",
      });
  ...
  const filteredApplications = applications.filter((app) => {
    ...
    const matchesTo =
      !filters.toDate || new Date(app.toDate) <= new Date(filters.toDate);
