...
const DepartmentList = () => {
  ...
  const fetchCurrentUser = async () => {
    try {
      const res = await fetch("https://facultyms-be-3.onrender.com/user", {
        method: "GET",
        credentials: "include",
  ...
  const fetchDepartments = async () => {
    try {
      const res = await axios.get("https://facultyms-be-3.onrender.com/departments", { withCredentials: true });
  ...
  const handleAdd = async (newDept) => {
    ...
    try {
      const res = await axios.post(
        "https://facultyms-be-3.onrender.com/departments",
  ...
  const handleEditSave = async (updatedDept) => {
    try {
      const res = await axios.put(
        `https://facultyms-be-3.onrender.com/departments/${updatedDept._id}`,
  ...
  const handleDelete = async (id, name) => {
    if (!isAdmin) return;
    try {
      await axios.delete(`https://facultyms-be-3.onrender.com/departments/${id}`, { withCredentials: true });
