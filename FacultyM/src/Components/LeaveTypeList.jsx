...
const LeaveTypeList = () => {
  ...
  const fetchLeaveTypes = async () => {
    try {
      setLoading(true);
      const res = await fetch("https://facultyms-be-3.onrender.com/leave-types", {
        method: "GET",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
      });
  ...
  const handleDelete = async (id) => {
    if (!isAdmin) return;
    try {
      const res = await fetch(`https://facultyms-be-3.onrender.com/leave-types/${id}`, {
        method: "DELETE",
        credentials: "include",
      });
      const data = await res.json();
  ...
  const handleEditSave = async () => {
    try {
      const res = await fetch(`https://facultyms-be-3.onrender.com/leave-types/${editModal}`, {
        method: "PUT",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
