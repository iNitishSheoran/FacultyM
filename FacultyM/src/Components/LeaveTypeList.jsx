...
const LeaveTypeList = () => {
  ...
  const fetchLeaveTypes = async () => {
    try {
      setLoading(true);
      const res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/leave-types`, {
        method: "GET",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
      });
  ...
  const handleDelete = async (id) => {
    if (!isAdmin) return;
    try {
      const res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/leave-types/${id}`, {
        method: "DELETE",
        credentials: "include",
      });
      const data = await res.json();
  ...
  const handleEditSave = async () => {
    try {
      const res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/leave-types/${editModal}`, {
        method: "PUT",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
